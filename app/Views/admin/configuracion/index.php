            ...
            <div>
                ...
                <?php if (!empty($config['tienda_logo'])): ?>
                    <div class="mb-3 flex items-center gap-3">
                        ...
                        <button type="button"
                                onclick="eliminarImagen('tienda_logo', '¿Eliminar el logo actual?')"
            ...
            <div>
                ...
                <?php if (!empty($config['tienda_favicon'])): ?>
                    <div class="mb-3 flex items-center gap-3">
                        ...
                        <button type="button"
                                onclick="eliminarImagen('tienda_favicon', '¿Eliminar el favicon?')"
                                class="text-xs text-red-500 hover:underline">Eliminar</button>
                    </div>
                <?php endif; ?>
            ...
            <div>
                ...
                <select name="fuente_principal" class="input">
                    ...
                    foreach ($fuentes as $f): ?>
                        <option value="<?= $f ?>" <?= ($config['fuente_principal'] ?? 'Inter') === $f ? 'selected' : '' ?>><?= $f ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div>
                ...
                <select name="fuente_titulos" class="input">
                    <?php foreach ($fuentes as $f): ?>
                        <option value="<?= $f ?>" <?= ($config['fuente_titulos'] ?? 'Inter') === $f ? 'selected' : '' ?>><?= $f ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div>
            ...
            <div class="md:col-span-2">
                ...
                <?php if (!empty($config['hero_imagen'])): ?>
                    <div class="mb-3 flex items-center gap-3">
                        ...
                        <button type="button"
                                onclick="eliminarImagen('hero_imagen', '¿Eliminar la imagen del hero?')"
                                class="text-xs text-red-500 hover:underline">Eliminar imagen</button>
                    </div>
                <?php endif; ?>
    ...
    <div class="bg-white rounded-2xl shadow-sm border p-6">
        ...
        <div class="grid grid-cols-1 gap-5">
            <div>
                ...
                <p class="text-xs text-gray-400 mt-1">Máx. 70 caracteres.</p>
            </div>
            <div>
                ...
                <p class="text-xs text-gray-400 mt-1">Máx. 160 caracteres.</p>
            </div>
            <div>
...
<script>
// Eliminar imagen sin form anidado
function eliminarImagen(campo, mensaje) {
    if (!confirm(mensaje)) return;
    document.getElementById('campo-eliminar').value = campo;
    document.getElementById('form-eliminar-imagen').submit();
}
